<h1>Fakitermelés</h1>

<h3>Szabad készlet</h3>



<table id="keszlet" class="table table-striped table-hover">
    <thead>
        <tr>
            <th>Fafaj</th>
            <th>Mennyiség</th>
            <th>Vágás dátuma</th>
            <th>Azonosító</th>
            <th>Művelet</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>Tölgy <img src="img/oak.png" style="height:1em;"></td>
            <td>20 m<sup>3</sup></td>
            <td>2016-02-12</td>
            <td>to160212</td>
            <td>
                <button type="button" class="btn btn-xs btn-primary">Szerkesztés</button>
                <button type="button" class="btn btn-xs btn-danger">Törlés</button>
            </td>
        </tr>
        <tr>
            <td>Bükk <img src="img/beech.png" style="height:1em;"></td>
            <td>100 m<sup>3</sup></td>
            <td>2016-02-12</td>
            <td>bu160212</td>
            <td>
                <button type="button" class="btn btn-xs btn-primary">Szerkesztés</button>
                <button type="button" class="btn btn-xs btn-danger">Törlés</button>
            </td>
        </tr>
        <tr>
            <td>Tölgy <img src="img/oak.png" style="height:1em;"></td>
            <td>20 m<sup>3</sup></td>
            <td>2016-02-12</td>
            <td>to160212</td>
            <td>
                <button type="button" class="btn btn-xs btn-primary">Szerkesztés</button>
                <button type="button" class="btn btn-xs btn-danger">Törlés</button>
            </td>
        </tr>
        <tr>
            <td>Cser <img src="img/turkey_oak.png" style="height:1em;"></td>
            <td>32 m<sup>3</sup></td>
            <td>2016-01-12</td>
            <td>cs160112</td>
            <td>
                <button type="button" class="btn btn-xs btn-primary">Szerkesztés</button>
                <button type="button" class="btn btn-xs btn-danger">Törlés</button>
            </td>
        </tr>
        <tr>
            <td>Gyertyán <img src="img/hornbeam.png" style="height:1em;"></td>
            <td>2 m<sup>3</sup></td>
            <td>2014-01-05</td>
            <td>gy140105</td>
            <td>
                <button type="button" class="btn btn-xs btn-primary">Szerkesztés</button>
                <button type="button" class="btn btn-xs btn-danger">Törlés</button>
            </td>
        </tr>
        <tr>
            <td>Tölgy <img src="img/oak.png" style="height:1em;"></td>
            <td>23 m<sup>3</sup></td>
            <td>2016-02-07</td>
            <td>to160207</td>
            <td>
                <button type="button" class="btn btn-xs btn-primary">Szerkesztés</button>
                <button type="button" class="btn btn-xs btn-danger">Törlés</button>
            </td>
        </tr>
        <tr>
            <td>Kőris <img src="img/ash.png" style="height:1em;"></td>
            <td>324 m<sup>3</sup></td>
            <td>2016-05-01</td>
            <td>ko160501</td>
            <td>
                <button type="button" class="btn btn-xs btn-primary">Szerkesztés</button>
                <button type="button" class="btn btn-xs btn-danger">Törlés</button>
            </td>
        </tr>
        <tr>
            <td>Akác <img src="img/acacia.png" style="height:1em;"></td>
            <td>112 m<sup>3</sup></td>
            <td>2016-04-21</td>
            <td>ak160421</td>
            <td>
                <button type="button" class="btn btn-xs btn-primary">Szerkesztés</button>
                <button type="button" class="btn btn-xs btn-danger">Törlés</button>
            </td>
        </tr>
    </tbody>
</table>

<h3>Új készlet felvitele</h3>
<form class="form-horizontal">
    <fieldset>

        <div class="jumbotron">


            <div class="form-group">
                <label class="col-md-4 control-label" for="selectbasic">Fafaj</label>
                <div class="col-md-4">
                    <select id="fafaj" name="selectbasic" class="form-control">
                        <option value="1">Akác</option>
                        <option value="2">Tölgy</option>
                        <option value="2">Gyertyán</option>
                        <option value="2">Bükk</option>
                        <option value="2">Cser</option>
                        <option value="2">Kőris</option>
                    </select>
                </div>
            </div>

            <!-- Appended Input-->
            <div class="form-group">
                <label class="col-md-4 control-label" for="appendedtext">Mennyiség</label>
                <div class="col-md-4">
                    <div class="input-group">
                        <input id="appendedtext" name="appendedtext" class="form-control" placeholder="szám" type="text">
                        <span class="input-group-addon">m<sup>3</sup></span>
                    </div>
                </div>
            </div>

            <div class="form-group">
                <label class="col-md-4 control-label" for="appendedtext">Vágás dátuma</label>
                <div class="col-md-4">
                    <input id="textinput" name="textinput" type="text" placeholder="éééé-hh-nn" class="form-control input-md datepicker">
                </div>
            </div>
            <div class="form-group">
                <label class="col-md-4 control-label" for="appendedtext">Azonosító</label>
                <div class="col-md-4">
                    <input id="textinput" name="textinput" type="text" placeholder="xx123456" class="form-control input-md">
                </div>
            </div>

            <div class="form-group">
                <label class="col-md-4 control-label" for="button1id"></label>
                <div class="col-md-8">
                    <button id="button1id" name="button1id" class="btn btn-success">Rögzítés</button>
                </div>
            </div>

        </div>

    </fieldset>
</form>
<h3>Függő megrendelések</h3>
<h4>Lejárt</h4>

<table id="keszlet" class="table table-striped table-hover" style="color:#933;">
    <thead>
        <tr>
            <th>#</th>
            <th>Fafaj</th>
            <th>Hossz</th>
            <th>Átmérő</th>
            <th>Mennyiség</th>
            <th>Ígért szállítási határidő</th>
            <th>Tényleges szállítási határidő</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>00001</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00002</td>
            <td><img src="img/turkey_oak.png" style="height:1em;margin-right:2px;">cser</td>
            <td>25-40 cm</td>
            <td>11-13 cm</td>
            <td>2 szórt űrméter</td>
            <td>2016-10-12</td>
            <td>2016-10-12</td>
        </tr>
        <tr>
            <td>00003</td>
            <td><img src="img/acacia.png" style="height:1em;margin-right:2px;">akác
                <br><img src="img/ash.png" style="height:1em;margin-right:2px;">kőris</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00004</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00005</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>

    </tbody>
</table>


<h4>Szeptember - 2016</h4>

<table id="keszlet" class="table table-striped table-hover">
    <thead>
        <tr>
            <th>#</th>
            <th>Fafaj</th>
            <th>Hossz</th>
            <th>Átmérő</th>
            <th>Mennyiség</th>
            <th>Ígért szállítási határidő</th>
            <th>Tényleges szállítási határidő</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>00001</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00002</td>
            <td><img src="img/turkey_oak.png" style="height:1em;margin-right:2px;">cser</td>
            <td>25-40 cm</td>
            <td>11-13 cm</td>
            <td>2 szórt űrméter</td>
            <td>2016-10-12</td>
            <td>2016-10-12</td>
        </tr>
        <tr>
            <td>00003</td>
            <td><img src="img/acacia.png" style="height:1em;margin-right:2px;">akác
                <br><img src="img/ash.png" style="height:1em;margin-right:2px;">kőris</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00004</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00005</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>

    </tbody>
</table>
<h4>Október - 2016</h4>

<table id="keszlet" class="table table-striped table-hover">
    <thead>
        <tr>
            <th>#</th>
            <th>Fafaj</th>
            <th>Hossz</th>
            <th>Átmérő</th>
            <th>Mennyiség</th>
            <th>Ígért szállítási határidő</th>
            <th>Tényleges szállítási határidő</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>00001</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00002</td>
            <td><img src="img/turkey_oak.png" style="height:1em;margin-right:2px;">cser</td>
            <td>25-40 cm</td>
            <td>11-13 cm</td>
            <td>2 szórt űrméter</td>
            <td>2016-10-12</td>
            <td>2016-10-12</td>
        </tr>
        <tr>
            <td>00003</td>
            <td><img src="img/acacia.png" style="height:1em;margin-right:2px;">akác
                <br><img src="img/ash.png" style="height:1em;margin-right:2px;">kőris</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00004</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00005</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>

    </tbody>
</table>
<h4>November - 2016</h4>

<table id="keszlet" class="table table-striped table-hover">
    <thead>
        <tr>
            <th>#</th>
            <th>Fafaj</th>
            <th>Hossz</th>
            <th>Átmérő</th>
            <th>Mennyiség</th>
            <th>Ígért szállítási határidő</th>
            <th>Tényleges szállítási határidő</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>00001</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00002</td>
            <td><img src="img/turkey_oak.png" style="height:1em;margin-right:2px;">cser</td>
            <td>25-40 cm</td>
            <td>11-13 cm</td>
            <td>2 szórt űrméter</td>
            <td>2016-10-12</td>
            <td>2016-10-12</td>
        </tr>
        <tr>
            <td>00003</td>
            <td><img src="img/acacia.png" style="height:1em;margin-right:2px;">akác
                <br><img src="img/ash.png" style="height:1em;margin-right:2px;">kőris</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00004</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00005</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>

    </tbody>
</table>
<h4>December - 2016</h4>

<table id="keszlet" class="table table-striped table-hover">
    <thead>
        <tr>
            <th>#</th>
            <th>Fafaj</th>
            <th>Hossz</th>
            <th>Átmérő</th>
            <th>Mennyiség</th>
            <th>Ígért szállítási határidő</th>
            <th>Tényleges szállítási határidő</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>00001</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00002</td>
            <td><img src="img/turkey_oak.png" style="height:1em;margin-right:2px;">cser</td>
            <td>25-40 cm</td>
            <td>11-13 cm</td>
            <td>2 szórt űrméter</td>
            <td>2016-10-12</td>
            <td>2016-10-12</td>
        </tr>
        <tr>
            <td>00003</td>
            <td><img src="img/acacia.png" style="height:1em;margin-right:2px;">akác
                <br><img src="img/ash.png" style="height:1em;margin-right:2px;">kőris</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00004</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00005</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>

    </tbody>
</table>
<h4>Január - 2017</h4>

<table id="keszlet" class="table table-striped table-hover">
    <thead>
        <tr>
            <th>#</th>
            <th>Fafaj</th>
            <th>Hossz</th>
            <th>Átmérő</th>
            <th>Mennyiség</th>
            <th>Ígért szállítási határidő</th>
            <th>Tényleges szállítási határidő</th>
        </tr>
    </thead>
    <tbody>
        <tr>
            <td>00001</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00002</td>
            <td><img src="img/turkey_oak.png" style="height:1em;margin-right:2px;">cser</td>
            <td>25-40 cm</td>
            <td>11-13 cm</td>
            <td>2 szórt űrméter</td>
            <td>2016-10-12</td>
            <td>2016-10-12</td>
        </tr>
        <tr>
            <td>00003</td>
            <td><img src="img/acacia.png" style="height:1em;margin-right:2px;">akác
                <br><img src="img/ash.png" style="height:1em;margin-right:2px;">kőris</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00004</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>
        <tr>
            <td>00005</td>
            <td><img src="img/oak.png" style="height:1em;margin-right:2px;">tölgy
                <br><img src="img/beech.png" style="height:1em;margin-right:2px;">bükk</td>
            <td>25-40 cm
                <br>12-15 cm</td>
            <td>11-13 cm
                <br>22-30 cm</td>
            <td>2 szórt űrméter
                <br>1 szórt űrméter</td>
            <td>2016-10-10</td>
            <td>2016-10-10</td>
        </tr>

    </tbody>
</table>